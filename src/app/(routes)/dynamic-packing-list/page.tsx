...
export default function TravelPreferencesForm() {
  ...
  const handleActivityAdd = () => {
    if (newActivity.name) {
      setActivityPreferences((prev) => ({
        ...prev,
        [newActivity.type]: [
          ...prev[newActivity.type as keyof ActivityPreferences],
          newActivity.name,
        ],
      }));
  ...
  const handleTravelHistoryChange = (
    index: number,
    field: keyof TravelHistory,
    value: string
  ) => {
    setTravelHistory((prev) =>
  ...
  const user_preferences = JSON.parse(localStorage.getItem("user-preferences"));
